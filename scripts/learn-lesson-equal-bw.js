...
const destinationDir = path.resolve(`results/${lessonType}-eq-i${iterations}/`);

if (fs.existsSync(destinationDir)) throw new Error('Destination lessonType-eq already exists.');
...
lessonNames.forEach((lessonName) => {
  ...
  jsonStream.on('data', ({ value }) => {
    if (Math.abs(value.input.reduce((p, c) => p + c - 0.5, 0)) > 0.09) return; // table is more than 1 pawn off balance
    lesson.push(value);
  });
